...
data pulses;
input initial aftermin smoking gender height weight activity;

pulsediff = aftermin -initial;
bmi = sqrt(weight/height**2);
...
label initial = "initial pulse" aftermin = "pulse after running for 1 minute"
	gender ="gender of person" pulsediff ="Difference in pulse";
	
format gender genderformat. activity activityformat. ;	

proc means data=pulses;
table initial aftermin pulsediff height weight;
run;

proc freq data=pulses;
table smoking gender activity bmicat;
run;

proc freq data=pulses;
table activity*bmicat;
run;
